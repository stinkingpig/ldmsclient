...
"LANDesk Management - APM - Policies - $PolicyName - Status = $PolicyStatus\n";
        ...
        $dbfile .= "\\Application\ Data\\LANDesk\\ManagementSuite\\Database\\LDClientDB.db3";
        $dbfile = Win32::GetShortPathName($dbfile);
		if (-e $dbfile) {

			if ($DEBUG) { Log("CallPolicyList: Searching in database $dbfile"); }	
	        my @rows;
	
    	    my $dbh = DBI->connect( "dbi:SQLite:dbname=$dbfile", "", "" ) or LogWarn("Can't open $dbfile: $!");
			...
			if ($dbh) {
	          ...
	          "select name,filename,description,status from PortalTaskInformation";
    		    ...
    		    my $sth = $dbh->prepare($sql) or LogWarn("Policy database prepare statement failed!: $!");
        		$sth->execute() or LogWarn("Policy database execute statement failed: $!");
...
sub CallLANDeskInfo {
    ...
    if ( -e $brokerconfigfile ) {
        ...
        my $brokerdata = $brokerxml->XMLin($brokerconfigfile) or LogWarn("Can't open $brokerconfigfile: $!");
		if ($brokerdata) {
...
"LANDesk Management - Broker Configuration Mode = Dynamically determine connection route\n";
	...
	if (-e "$psdir\\preferredserver.dat") {
		...
		print FILE
"LANDesk Management - Preferred Server = $psentry\n";
	}
	if (-e "$psdir\\preferredservers.dat") {
		...
		close PSFILE;
		print FILE
"LANDesk Management - Preferred Servers = $psentries\n";
	}
	$psentries = "";
	$RegKey = $Registry->{"HKEY_LOCAL_MACHINE/Software/LANDesk/ManagementSuite/WinClient/SoftwareDistribution"};
	if ($RegKey) {
...
sub CallEnumerateGroups {
    ...
    foreach $Group ( in $GroupList) {
		...
		if ($Group->Name) {
		    ...
		    print FILE "Local Users and Groups - Local Groups - $GroupName - Members = $GroupMembers\n";
		}
    }
}
