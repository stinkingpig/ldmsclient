...
sub CallDCCUWol {
    ...
    if (! -e $getwoldat ) {
        if ( -e $getwol ) {
            ...
            if ( $? != 0 ) { &LogWarn("Getwol.exe run error: $! $?"); }
        }
    }
    ...
    if ( -e $getwoldat ) {
        ...
        close $GETWOLDAT;
        if ( $wolresult == 1 ) {
          print $FILE "BIOS - WakeOnLAN = On\n";
        }
        if ( $wolresult == 0 ) {
            print $FILE "BIOS - WakeOnLAN = Off\n";
        }
        unlink($getwoldat)
            or &Logwarn("Cannot delete temp file $getwoldat : $!");
    }
    else {
