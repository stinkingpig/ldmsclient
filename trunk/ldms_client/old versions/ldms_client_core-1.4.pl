...
#############################################################################
#
# TODO -- HKLM Reader, easier to use than LDAPPL3.INI and could support more
# key types (such as Binary)
# TODO -- Simple File sharing status (enabled / disabled) based on:  HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa\forceguest
...
if ($DEBUG) {
    &Log(
"From config file: Battery=$Battery, Netstat=$Netstat, PolicyList=$PolicyList, FindPST=$FindPST, NICDuplex=$NICDuplex, EnumerateGroups=$EnumerateGroups, LANDeskInfo=$LANDeskInfo, RegistryReader=$RegistryReader, RegistryInfo is $RegistryInfo, Produkey=$Produkey, SID=$SID, Macintosh=$Macintosh"
    );
	if ($RegistryReader) {
...
if ( defined($content) ) {
    ...
    if ( $onlineversion != $ver ) {
        &LogWarn(
"Update available at http://www.droppedpackets.org/inventory-and-slm/ldms_client"
        );
		$update_available = 1;
    }
}
...
if ($DEBUG) {
    &Log(
"From user input: Battery=$Battery, Netstat=$Netstat, PolicyList=$PolicyList, FindPST=$FindPST, NICDuplex=$NICDuplex, EnumerateGroups=$EnumerateGroups, LANDeskInfo=$LANDeskInfo, RegistryReader=$RegistryReader, RegistryInfo=$RegistryInfo, Produkey=$Produkey, SID=$SID, Macintosh=$Macintosh"
    );
	if ($RegistryReader) {
		&Log("RegistryReader targets: @rr");
	}
...
sub Show_MainWindow {
	...
	if ($update_available) {
		$sb->SetText(0, "Update available. click Help to go to the website.");
	}
