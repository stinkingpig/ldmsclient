...
# TODO -- Should download the file with sdclient from http, read it, then delete it after inventory scanning is complete.
my $configfileresult = `$ldclient/sdclient.exe /f /p=http://$core/ldlogon/ldms_client.ini"`;
my $Config = Config::Tiny->new();
$Config = Config::Tiny->read("$ldclient/ldms_client.ini")
  || &LogDie( "Can't read $ldclient/ldms_client.ini: ",
    Config::Tiny->errstr() );
...
sub CallEnumerateGroups {
    ...
    my ( $GroupList, $Group, $GroupName, $MemberList, $Member, $MemberName,
        $GroupMembers );
    my $Members = "";
...
sub CallRegistryReader() {
    ...
    if ( -e $startasuser ) {
        if ( -e $ldms_client_regreader ) {
			...
			open (RRTEMP, ">$rrtemp") or &LogWarn("Cannot open $rrtemp for writing: $!");
            ...
            for my $rri ( 1 .. 10 ) {
               ...
               # then write the value out
                if ( my $hkcukey = $rr[$rri] ) {
					...
					if ($DEBUG) { &Log("Wrote processed input of $hkcukey, $subkey to $rrtemp"); }
				}
			}
			close(RRTEMP);
            if ($DEBUG) {
...
`$startasuser ///timeout=10 ///silent $ldms_client_regreader -keyfile="$rrtemp"`;
            ...
            if (!$hkcuresult) {
				...
				open (RRTEMP, "<$rrtemp") or &LogWarn("Cannot open $rrtemp for reading: $!");
				while (<RRTEMP>) {
				...
				close(RRTEMP);
				unlink($rrtemp) or &LogWarn("Cannot delete temp file $rrtemp: $!");
            }
        }
        else {
...
sub CallRegistryInfo() {
	...
	my $HKLMKey = $Registry->{"HKEY_LOCAL_MACHINE/"} or &LogWarn("Cannot open the HKLM hive for reading! $^E");
    # Read in the .ini keys I need to look for
    for my $rii ( 1 .. 10 ) {
        ...
        if ( my $hklmkey = $ri[$rii] ) {
			...
			if (defined($hklmkey) && defined ($subkey)) {
				if ($DEBUG) { &Log("Read processed input of $hklmkey, $subkey from configuration file"); }
				my $value;
				if ($RegKey->{$hklmkey}->{$subkey}) {
		    		$value = $RegKey->{$hklmkey}->GetValue("$subkey");
					if ($DEBUG) { &Log("Read output of $value"); }
				} else {
					if ($DEBUG) { &Log("Found nothing at $hklmkey $subkey"); }
					$value = "NULL";
				}
			print FILE "Custom Data - HKLM - $hklmkey - $subkey = $value\n";
			}
		}
	}
}
...
sub CallProdukey() {
    ...
    if ( -e $produkey ) {
		...
		if ($pk_version < 126) { 
			LogWarn("CallProdukey: $produkey is too old, should be 1.2.6 or better");
			return 1;
		}
        ...
        if ( -e $produkeydat ) {
			...
			open( PKDFILE, "<$produkeydat" ) or &LogWarn("Can't open $produkeydat: $!");
			while (<PKDFILE>) {
				my ($pk_name, $pk_id, $pk_key, $pk_install, $pk_sp, $pk_machine) = split(/,/);
				if ($pk_sp) {
					$pk_name = $pk_name." ".$pk_sp;
				}
				print FILE
"Licenses - $pk_name - product_id = $pk_id\n";
            	print FILE
"Licenses - $pk_name - product_key = $pk_key\n";
	            print FILE
"Licenses - $pk_name - installation_folder = $pk_install\n";
			}
			close(PKDFILE);
			unlink($produkeydat) or &Logwarn("Cannot delete temp file $produkeydat: $!");
        }
    }
    else {
