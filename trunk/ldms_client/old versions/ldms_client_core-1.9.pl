...
my (
    ...
    $FindNSF,   $FindProfileSize, $AggroMailSearch, $ProdukeyBinary,
    $DCCUWol,   $DCCUWolBinary
);
...
my (
    ...
    $lbl_AggroMailSearch, $lbl_DCCUWol
);
...
if ( -e $configfile ) {
    ...
    $Produkey        = $Config->{_}->{DCCUWol};
    $ProdukeyBinary  = $Config->{_}->{DCCUWolBinary};
    $SID             = $Config->{_}->{SID};
...
if ($DEBUG) {
    &Log(   "From config file: Battery=$Battery, Netstat=$Netstat, "
          ...
          . "Produkey=$Produkey, SID=$SID, Macintosh=$Macintosh, "
          . "FindNSF=$FindNSF, FindProfileSize=$FindProfileSize, "
          . "AggroMailSearch=$AggroMailSearch, ProdukeyBinary=$ProdukeyBinary"
          . "DCCUWol=$DCCUWol, DCCUWolBinary=$DCCUWolBinary"
    );
    if ($RegistryReader) {
