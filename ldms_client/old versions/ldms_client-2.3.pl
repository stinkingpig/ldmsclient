...
# TODO: Gather all logins from primary owner history
...
my ( $totalpstcount, $totalnsfcount ) = 0;
...
my $configfile = "\\\\$core\\ldlogon\\ldms_client.ini";
if ( !-e $configfile ) {
    if ($DEBUG) { &Log("Didn't see $configfile, trying with sdclient."); }
    ...
    if ( !-e $configfile ) {
        if ($DEBUG) { &Log("Didn't see $configfile, trying in ldclient."); }
        $configfile = $ldclient . "\\ldms_client.ini";
        if ( !-e $configfile ) {
...
print $FILE "Custom Data - $prog - version = $ver\n";
...
"HKEY_LOCAL_MACHINE/System/Currentcontrolset/Control/Class/{4D36E972-E325-11CE-BFC1-08002bE10318}"
    ...
    foreach my $nic (@nic_names) {
        ...
        if ( $DriverName eq '1394 Net Adapter' ) {
            if ($DEBUG) { &Log("CallNICDuplex: Skipping $DriverName"); }
            next;
        }
        ...
        if ($DuplexMode) {
            if ($DEBUG) {
                &Log(   "CallNICDuplex: $DriverName seems to be "
                      . "a Realtek or 3Com" );
            }
            $drivertype = 1;
        }
        ...
        if ($DuplexMode) {
            if ($DEBUG) {
                &Log( "CallNICDuplex: $DriverName seems to be " . "an Intel" );
            }
            $drivertype = 1;
        }
        ...
        if ($DuplexMode) {
            if ($DEBUG) {
                &Log(
                    "CallNICDuplex: $DriverName seems to be " . "a Broadcom" );
            }
            $drivertype = 2;
        }
...
"LANDesk Management - Broker Configuration Mode = Dynamically determine connection route\n";
        ...
        if ($DEBUG) {
            &Log("CallLANDeskInfo: $brokerconfigfile not present.");
        }
    }
    ...
    if ( -e "$psdir\\preferredserver.dat" ) {
        ...
        if ($DEBUG) {
            &Log("CallLANDeskInfo: 8.7.1 client, $psfile is present.");
        }
    ...
    if ( -e "$psdir\\preferredservers.dat" ) {
        ...
        if ($DEBUG) {
            &Log("CallLANDeskInfo: 8.7.2 or later client, $psfile is present.");
        }
        open( $PSFILE, '<', "$psfile" ) or &LogWarn("Can't open $psfile : $!");
...
"HKEY_LOCAL_MACHINE/Software/LANDesk/ManagementSuite/WinClient/SoftwareDistribution"
    ...
    if ($RegKey) {
        ...
        if ( $psentries && $DEBUG ) {
            &Log("CallLANDeskInfo: $psentries present in Registry.");
        }
    }
    return 0;
}
...
sub CallFindPST {
    ...
    print $FILE "Email - PST Files - Number of Files = $totalpstcount\n";
    return 0;
}
...
sub CallFindNSF {
    ...
    print $FILE "Email - NSF Files - Number of Files = $totalnsfcount\n";
    return 0;
}
                ...
                if ($HKLMKey) {
                    ...
                    if ( defined($value) ) {
                            ...
                            or $type eq "REG_BINARY" )
                        {
                            ...
                            &LogWarn(
                                "$hklmkey $subkey is an unsupported type: $type"
                            );
                        }
...
sub CallDCCUWol {
    ...
    my $getwoldat = lc( dirname($getwol) . "\\DCCUResults_SUCCESS.xml" );
    if ( !-e $getwoldat ) {

        if ($DEBUG) { &Log("CallDCCUWol: Didn't find $getwoldat"); }

        # The 3.x DCCU file
        $getwoldat = lc( dirname($getwol) . "\\TaskResult.xml" );
    }
                ...
                if ( $line =~ /WakeupOnLAN\"/ix ) {
                    ...
                    foreach my $property (@properties) {
                        if ( $property =~ m/value/ix ) {
                            ...
                            if ($DEBUG) {
                                &Log(   "CallDCCUWol: found $wolresult "
                                      . "from $line" );
                            }
                        }
                    }
