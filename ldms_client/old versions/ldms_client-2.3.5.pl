...
"can't find core server name in HKLM/Software/Intel/LANDesk/LDWM CoreServer."
    ...
    if ( !-e $output ) {
        ...
        if ( !-e $output ) {
            if ($DEBUG) { &Log("Ran $sdclient, but didn't see $output, "
...
sub ReadBrokerSettings {
    ...
    if ( -e $brokerconfigfile ) {
        ...
        &ReportToCore(
            "LANDesk Management - Broker Configuration Mode = "
...
"HKEY_LOCAL_MACHINE/Software/LANDesk/ManagementSuite/WinClient/SoftwareDistribution"
    ...
    if ($DEBUG) { &Log("ReadPreferredServers: finished");
            }
    return 0;
}
...
sub CallFindPST {
    ...
    &ReportToCore("Email - PST Files - Total Disk Size = $totalpstsize");
...
sub CallFindNSF {
    ...
    &ReportToCore("Email - NSF Files - Total Disk Size = $totalnsfsize");
...
sub CallFindProfileSize {
    ...
    for my $user ( glob( $userdir . '/*' ) ) {
        if ( -d $user ) {
            ...
            &ReportToCore("Profile Size - $user = $size");
        }
    }
    return 0;
}
...
sub ProcessPSTFile {
    ...
    &ReportToCore("Email - PST Files - $_ - File Size = $pstfilesize");
...
sub ProcessNSFFile {
    ...
    &ReportToCore("Email - NSF Files - $_ - File Size = $nsffilesize");
...
"Wrote processed input of $hkcukey, $subkey to $rrtemp"
            ...
            while (<$RRTEMP>) {
                ...
                if ($DEBUG) { &Log("hkcukey is $hkcukey, "
                        . "subkey is $subkey, "
                        . "value is $value"); 
                }
                chomp($subkey);
...
sub FindGetwoldat {
    ...
    if ($DCCUWolBinary) { push( @searchpaths, $DCCUWolBinary ); }
    ...
    $searchpath = $WINDIR . "\\system32";
    push( @searchpaths, $searchpath );

    foreach my $path (@searchpaths) {

        # Look for the old file first
        $searchfile = $searchpath . "\\DCCUResults_SUCCESS.xml";
        push( @candidates, $searchfile );

        # Look for the new file next
        $searchfile = $searchpath . "\\TaskResult.xml";
        push( @candidates, $searchfile );
    }
    ...
    for ( 0 .. $#candidates ) {
        if ( !-e $candidates[$_] ) {
            if ($DEBUG) { &Log("FindGetwoldat: removing $candidates[$_]"); }
            delete $candidates[$_];
        }
        else {
            ...
            $ages{ $candidates[$_] } = localtime() - $ctime;
        }
    }
    ...
    foreach my $possible (@candidates) {
        ...
        if ( $ages{$possible} < $timediff ) {
            ...
            # If there's a newer one, or this is really old, might as well
            # delete it
            unlink $possible;
        }
    }
...
sub CallSID {
    ...
    if (length($sidstring) < 10) { 
        if ($DEBUG) { &Log("CallSID: got corrupt SID: $sidstring"); 
        } 
    } else {
...
sub ReportToCore {
    ...
    if ($msg) {
        ...
        &Log("ReportToCore called with nothing to do!");
        return 1;
    }
}
### trimkey subroutine ######################################################
