...
sub FindGetwoldat {
    ...
    for ( 0 .. $#candidates ) {
        # Sometimes a blank will get in there... 
        if ( length($candidates[$_]) < 1 ) {
            delete $candidates[$_];
        }
        if ( !-e $candidates[$_] ) {
    ...
    foreach my $possible (@candidates) {
        ...
        if ( $ages{$possible} < $timediff ) {
            if ($DEBUG) {
                &Log("$possible is more recent at $ages{$possible} than $timediff");
            }
...
sub CallSID {
    ...
    if ( length($sidstring) < 10 ) {
        if ($DEBUG) {
            &Log("CallSID: got corrupt SID: $sidstring");
        }
    }
    else {
