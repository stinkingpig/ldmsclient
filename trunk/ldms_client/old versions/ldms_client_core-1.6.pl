...
sub Show_MainWindow {
    ...
    $form_AggroMailSearch = $main->AddCheckbox(
        -name    => "aggromailsearch_field",
        -checked => $AggroMailSearch,
        -tabstop => 1,
        -pos     => [ $leftmargin, $nexthoriz += 25 ],
        -size => [ 15, 20 ],
    );
    $lbl_AggroMailSearch = $main->AddLabel(
        ...
        -text => "Search the whole drive instead of just profile directories",
        -pos  => [ $leftmargin + 20, $nexthoriz + 3 ],
        -size => [ 300, 20 ],
    );
...
sub Default_Click {
    ...
    $AggroMailSearch = $form_AggroMailSearch->Checked();
