...
my @rr;
$rr[1]  = $Config->{RegistryReader}->{1};
$rr[2]  = $Config->{RegistryReader}->{2};
$rr[3]  = $Config->{RegistryReader}->{3};
$rr[4]  = $Config->{RegistryReader}->{4};
$rr[5]  = $Config->{RegistryReader}->{5};
$rr[6]  = $Config->{RegistryReader}->{6};
$rr[7]  = $Config->{RegistryReader}->{7};
$rr[8]  = $Config->{RegistryReader}->{8};
$rr[9]  = $Config->{RegistryReader}->{9};
$rr[10] = $Config->{RegistryReader}->{10};
...
sub CallRegistryReader() {
    ...
    if ( -e $startasuser ) {
        if ( -e $ldms_client_regreader ) {
            ...
            if ($DEBUG) {
             	Log(
"CallRegistryReader: $startasuser ///timeout=10 ///silent $ldms_client_regreader -keys=\"$rrtemp\"."
                );
            }
            my $hkcuresult =
`$startasuser ///timeout=10 ///silent $ldms_client_regreader -keys="$rrtemp"`;
